...
public class EventMapper {
    ...
    public static Result<Event> toDomain(com.eventer.admin.model.Event model) {
        ...
        if (categoriesOrError.stream().anyMatch(Result::isFailure)) {
            return Result.fromError(categoriesOrError.stream().filter(Result::isFailure).findFirst().get());
        }

        Result<Event> eventOrError = Event.create(model.getId(), model.getTitle(), model.getDescription(),
                model.getLocation(),
                model.getWeatherConditionAvailability(),
                categoriesOrError.stream().map(Result::getValue).collect(Collectors.toSet()));
    ...
    public static Result<Page<Event>> toDomainPage(Page<com.eventer.admin.model.Event> foundEvents) {
        ...
        Page<Event> result = new PageImpl<>(
